...
export function ImageGalleryCarousel({ images, title, className }: Props) {
  ...
  if (!items.length) {
    return (
      <div className={`flex items-center justify-center rounded-[14px] bg-slate-100 text-slate-400 ${className || "aspect-[16/11]"}`}>
    ...
    <>
      ...
      {fullScreen ? (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 p-3 md:p-8" role="dialog" aria-modal="true" aria-label={`Galeri ${title}`}>
          <button className="absolute right-4 top-4 z-10 rounded-[14px] bg-black/60 p-2 text-white" type="button" onClick={() => setFullScreen(false)} aria-label="Tutup galeri">
            <X className="h-6 w-6" />
          </button>
...
function GalleryFrame({ current, images, title, fullScreen, onOpen, onPrevious, onNext, onSelect, onTouchStart, onTouchEnd, className }: FrameProps) {
  const [failed, setFailed] = useState<number[]>([]);

  return (
    <div className={`relative overflow-hidden bg-slate-100 ${fullScreen ? "h-[82vh] rounded-[14px]" : className || "aspect-[16/11] rounded-promo"}`} onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
      ...
      {images.length > 1 ? (
        <>
          <div className="absolute right-3 top-3 flex items-center gap-1 rounded-[8px] bg-black/60 px-2.5 py-1 text-xs font-semibold text-white">
...
function Arrow({ direction, onClick }: { direction: "left" | "right"; onClick: () => void }) {
  ...
  return (
    <button
      ...
      className={`absolute top-1/2 -translate-y-1/2 rounded-[14px] bg-black/45 p-1.5 text-white transition hover:bg-black/65 ${direction === "left" ? "left-2" : "right-2"}`}
    >
